  ...
  const inviteToGroup = async (req, res) => {
    try {
      ...
      if (!group.members.some((m) => m.toString() === inviterId)) {
        return res
          .status(403)
          .json({ message: "You must be a member to invite others" });
      }
  ...
  const updateGroup = async (req, res) => {
    try {
      ...
      if (!isCreator && !isAdmin) {
        return res
          .status(403)
          .json({ message: "Only creator or admin can update the group" });
      }
  ...
  const toggleAdmin = async (req, res) => {
    try {
      ...
      if (group.creator.toString() !== userId) {
        return res
          .status(403)
          .json({ message: "Only the creator can manage admins" });
      }
      ...
      if (memberId === group.creator.toString()) {
        return res
          .status(400)
          .json({ message: "Cannot change creator admin status" });
      }
      ...
      const isCurrentlyAdmin = group.admins.some(
